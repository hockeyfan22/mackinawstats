...
```{r}
hex = nhl %>% 
  ggplot(aes(xCordAdjusted, yCordAdjusted)) +
  geom_hex(size = 1 , color = "black") +
...
par(mfrow = c(1, 1))
plot_gg(hex, width = 5, height = 4, scale = 300, multicore = TRUE, windowsize = c(1200, 960),
...
nhl %>% 
  ggplot(aes(xCordAdjusted, yCordAdjusted)) +
  geom_point(aes(size = xGoal) , color = "black") +
  scale_fill_viridis_c(option = "C") +
  gg_rink()
```
