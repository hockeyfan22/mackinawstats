...
```{r}
game = read_csv('https://evolving-hockey.com/session/4ad510adaea95d99ca75e785864f1e63/download/pbp_query_download?w=')
```
```{r}
...
game2  %>% 
  ...
  ggplot(aes(coords_x, coords_y, z = density)) +
  geom_contour_filled(aes(fill = stat(density))) + 
  guides(fill = guide_colorsteps(barheight = unit(10))) +
  gg_rink(side = "right") +
  gg_rink(side = "left") +
  ...
  labs(title = "TOR Vs. PIT, Shots Heatmap", subtitle = "Data from Moneypuck", caption = "Chart by @MackinawStats")
```
